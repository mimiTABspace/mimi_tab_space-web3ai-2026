...
銀行残高から家族メンバーへコインを送付できる機能を追加。

- 送付上限: 1日100コインまで
- 送付時にメッセージを添付可能、受取時に返信できる
- ホーム起動時に受取/送信の確認モーダルを表示
- これに伴いアプリバージョンを v0.5.0 に更新

---

### 銀行利率の3期間連続対策

コイン銀行の利率が偶然3期間（朝/夕）連続で同じ値になってしまうのを防ぐ処理を追加。

- 直前2期間の利率が同じ値で、かつ今回算出した利率もそれと一致してしまう場合のみ、別の値になるまで振り直す
- 表示用の利率計算（ホーム・ショップの利率表示、明日の利率予報、利率グラフ）と、実際の利息付与処理の両方に同一ロジックを適用し、表示と実際の付与額が食い違わないようにした
