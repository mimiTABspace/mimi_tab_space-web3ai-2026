...
## 追加更新（2026-07-16、SCQuest v0.7.0のまま・バージョン番号は据え置き）

### 種族限定おやつ機能の実装反映

2026-07-13に実装・動作確認まで済んでいたが未コミットのまま残っていた「種族限定おやつ」機能をコミット。

- `game_status` テーブルに `treat_{dog,cat,rabbit,bird,tabcat}_count` 列を追加（`supabase/species_treats_setup.sql`）
- ショップ（`/shop`）: レベル解放条件（Lv1/5/15/30、タブネコのみ4種なつき度MAXで解放）付きで5種類の種族限定おやつを販売
- 相棒ページ（`/pet`）: 所持していれば「種族限定おやつをあげる」ボタンを表示。種族が一致すればなつき度+8/XP+20（通常のおやつは+5/+15）、不一致の場合はアイテムを消費せず「この子には効かないみたい…」と表示するのみ

### くじ（ガチャ）機能をデバッグモード限定から全ユーザーに公開

これまで通常ユーザーがくじタブを開くと「店主が仕入れの旅に出ています…」というプレースホルダーが表示され、開発者のみが使えるデバッグモードでないと実際のガチャ画面（排出確率・くじを引くボタン・履歴）が使えない状態だった。動作確認が済んでいたにもかかわらず一般公開する変更が漏れていたため、`app/shop/page.tsx` の `isDebugMode()` による分岐を削除し、全ユーザーが遊べるように修正。

### 細かい修正

- セール日（毎月1・15日、20%OFF）に複数個まとめて購入する際、単価ごとに切り捨てていたため合計金額が実際より高くなっていた計算方法を修正。合計してから切り捨てる方式（`saleTotal`）に変更
- 相棒ページのステータス表示で、連続記録日数（🔥）だけ他のアイコン（⭐レベル・💰コイン・❤なつき度、いずれもアイコンコンポーネント）と違い生の絵文字を使っていたため、行の高さが揃わず数値が下にズレて見えていた表示不具合を修正。アイコンの高さを18pxに統一
- ショップの直接購入タブに残っていた「購入したおやつは相棒ページから使えます（近日実装予定）」という古い注意書きを削除。相棒ページでの給餌機能（通常おやつ・種族限定おやつとも）は既に実装済みのため不要になっていた
- 種族限定おやつがロック中（レベル未到達）のとき「🐶イヌ限定」のような対象種族ラベルがそのまま見えてしまい、レベルアップ前から中身が分かってしまっていたため「？？？限定」に隠すよう修正。解放後は元のラベルを表示
